...
def _topk(scores, K=40):
    # https://cloud.tencent.com/developer/article/1690700
    ...
    topk_ys   = (topk_inds / width).int().float()
    topk_xs   = (topk_inds % width).int().float()
    # to shape: [batch , class * K] 这样的结果是不分类别的，全体class中最大的100个
    topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K)
    # topk_score
    # 形状 【batch, K】, 通过gather_feature方法获取，其保存的是全部类别前100个最大的score。
    # topk_ind
    # 形状 【batch, K】，代表通过topk调用结果的下角标, 其保存的是全部类别对应的100个score的下角标。
    topk_clses = (topk_ind / K).int()
    # 相当于是feat根据ind的角标的值获取到了对应feat位置上的结果。最终feat形状为【batch，100】
...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    # 推理过程核心工作就是从heatmap提取得到需要的bounding box，
    # 具体的提取方法是使用了一个3x3的最大化池化，检查当前热点的值是否比周围8个临近点的值都大。然后取100个这样的点，再做筛选。
    batch, cat, height, width = heat.size()

    # heat = torch.sigmoid(heat)  # 归一化到0-1
    # perform nms on heatmaps
    heat = _nms(heat)
    #
