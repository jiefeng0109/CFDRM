...
class kp_module(nn.Module):
    ...
    def __init__(
        ...
        self.max1 = make_pool_layer(curr_dim)  # 无用，没有下采样两倍
        ...
        # print(self.low1)
        # 二阶Hourglass就是把一阶模块的中间的残差模块替换成一个一阶Hourglass模块。以此类推，
        self.low2 = kp_module(
            n - 1, dims[1:], modules[1:],  # 循环调用kp_module,即低一阶的Hourglass模块结构，这三个重点
            layer=layer,
            ...
            **kwargs
            ...
            make_low_layer(  # 跳出循环    # next_mod x residual，next_dim -> next_dim -> ... -> next_dim
...
class exkp(nn.Module):
    ...
    def __init__(
        ...
        self.pre = nn.Sequential(
            convolution(7, 9, 128, stride=1),  # todo  # 源码 7,3,128,stride=2  # 快速降维为原来的1/4
        ...
        # keypoint heatmaps
        # 3 head(heatmap/xy/wh)
        for head in heads.keys():
            if 'hm' in head:
                module = nn.ModuleList([
                    make_heat_layer(
                        cnv_dim, curr_dim, heads[head])  # heatmap输出通道为num_classes
                    for _ in range(nstack)
                ])
                self.__setattr__(head, module)
                for heat in self.__getattr__(head):
                    heat[-1].bias.data.fill_(-2.19)  # -2.19是focal loss中的默认参数，论文的4.1节有详细说明，-ln((1-pi)/pi),这里的pi取0.1
            else:
                module = nn.ModuleList([
                    make_regr_layer(
                        cnv_dim, curr_dim, heads[head])
                    for _ in range(nstack)
                ])
                self.__setattr__(head, module)
    ...
    def forward(self, image):
        # print('image shape', image.shape) # 4,9,512,512
        inter = self.pre(image)
        # print(inter.shape) 4,256,256,256
        outs = []
        ...
        return outs
    # outs实际上是几个hourglass，几个输出结果，最后算loss的时候算的是最后一个的output
