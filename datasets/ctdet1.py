...
class CTDetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        post = img[:, :, 6:9]
        img = img[:, :, 3:6]
        # neg = neg.reshape(neg.shape[0], neg.shape[1], 1)  # 新增加1维
        # print(neg.shape[0])
        # print(np.where(neg!=1))
        # neg_multi = np.ones((neg.shape[0], neg.shape[1], 3))
        # neg_multi[:, :, 0:1] = neg
        # neg_multi[:, :, 1:2] = neg
        # neg_multi[:, :, 2:3] = neg
        # print(neg_multi.shape)
        ...
        inp_post = cv2.warpAffine(post, trans_input,
                                  (input_w, input_h),
                                  flags=cv2.INTER_LINEAR)
        # neg_feature = cv2.warpAffine(neg_multi, trans_input,
        #                      (input_w, input_h),
        #                     flags=cv2.INTER_LINEAR)
        # print(inp.shape)
        ...
        hm_w = np.ones((num_classes, output_h, output_w), dtype=np.float32)
        ...
        for k in range(num_objs):  # 按实际有多少个目标来
            ...
            # print('------------------------------')
            # print(ann['weight'])
            weight = ann['weight']  # e^x  范围 1-2.7
            # print(weight)
            # weight = 1
            if flipped:
            ...
            if h > 0 and w > 0:
                ...
                # print(hm[cls_id].shape)
                # print(np.where(hm[cls_id])!=0)
                # hm_w[cls_id][int(min(bbox[0], 0)):int(max(bbox[2], output_w)), int(min(bbox[1], 0)):int(max(bbox[3], output_h))] = weight
        ...
        for neg_sample in neg_samples_pic:
            if neg_sample[1] >= left and neg_sample[2] >= upper \
                ...
                # 找到与训练集对应的neg bboxes
                bbox = np.array([neg_sample[1]-left, neg_sample[2]-upper, neg_sample[3]-left, neg_sample[4]-upper],
                                dtype=np.float32)
                weight = random.uniform(1, 2.7)
                if flipped:
                    bbox[[0, 2]] = width - bbox[[2, 0]] - 1
                # '输出的仿射变换，这些仿射变换由相应的函数完成，在实际编写中只需确定诸如中心点c、长边长度s等参数，便于在自己的数据集中使用'
                # 左上和右下进行仿射变换
                ...
                if h > 0 and w > 0:
                    ...
                    int(min(bbox[1], 0)):int(max(bbox[3], output_h))] = weight

        # print(hm_w[np.where(hm_w!=0)])
        # print(hm[np.where(hm!=0)])
        # ret = {'input': inp, 'hm': hm, 'hm_w': hm_w,  'ind': ind, 'wh': wh}
        ret = {'input': inp, 'hm': hm, 'hm_w': hm_w, 'hm_neg': hm_neg, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh}
