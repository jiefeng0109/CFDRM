...
class CTDetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for k in range(num_objs):  # 按实际有多少个目标来
            ...
            # 课程学习
            if self.epoch <=8:
                if weight < self.weight2:  # 最大置信度的1/3训练
                    continue
            elif 16 >= self.epoch > 8:
                if weight < self.weight1:
                    continue
            ...
            if h > 0 and w > 0:
                ...
                hm_pos[cls_id][int(min(bbox[0], 0)):int(max(bbox[2], output_w)),
                int(min(bbox[1], 0)):int(max(bbox[3], output_h))] = math.exp(weight)
        ...
        ret = {'input': inp, 'hm': hm, 'hm_w': hm_w, 'hm_pos': hm_pos, 'hm_neg': hm_neg, 'reg_mask': reg_mask,
               'ind': ind, 'wh': wh}
