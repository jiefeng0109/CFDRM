        ...
        if epoch < eada: # 直接替代
            ...
            for annotation in pseudo_gts:
                ...
                for obj in objs:
                    ...
                    # 固定值0.1
                    if bbox[0] < point[0] < bbox[2] and bbox[1] < point[1] < bbox[3] \
                            and new_weight >= old_weight and new_weight > 0.5:  # tp = 0.5
                        # print(annotation)
                        # update
                        w1, w2 = old_weight/(old_weight+new_weight), new_weight/(old_weight+new_weight)
                        print(w1, w2)
                        x1 = w1 * bbox2[0] + w2 * bbox[0]
                        y1 = w1 * bbox2[1] - w2 * bbox[1]
                        x2 = w1 * bbox2[2] + w2 * bbox[2]
                        y2 = w1 * bbox2[3] + w2 * bbox[3]
                        annotation.update({
                            "area": (x2 - x1) * (y2 - y1),
                            "bbox": [x1, y1, x2-x1, y2-y1],
                            "weight": (new_weight + old_weight)/2
                        })
                update_coco.append(annotation)
