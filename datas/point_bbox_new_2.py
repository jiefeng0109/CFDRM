...
bboxes_video = []
output = []
neg_sample = []   # save negative samples bounding boxes
...
for i, file in enumerate(os.listdir(load_path)):
    ...
    gt = [car for car in data if car[0] == i] # 点标注
    print(len(gt))
    print(gt)
...
np.savetxt('..\\datas\\MOT\\' + dataset +'neg_samples.txt', np.array(neg_sample), delimiter=',', fmt='%d')  # todo
