...
def crop_img(load_path=r'E:\Jiang\data\MOT\\',
    ...
    for dataset in datasets:
        ...
        if dataset =='SkySat\\002\\':
            ...
            #                   dtype=int).tolist()  # todo
        elif dataset == 'SD\\4570-3360\\':
            data = np.loadtxt(r'datas\\MOT\\' + dataset + 'pseudo_bbox.txt', delimiter=',',
                              dtype=int).tolist()  # todo
        elif dataset == 'SD\\8450-2180\\':
            data = np.loadtxt(r'datas\\MOT\\' + dataset + 'pseudo_bbox.txt', delimiter=',',
                              dtype=int).tolist()  # todo
        elif dataset =='DXB_train_val_test\\1960-2690\\':
            data = np.loadtxt(r'datas\\MOT\\DXB\\1960-2690\\pseudo_bbox.txt', delimiter=',',
                              dtype=int).tolist()  # todo
        elif dataset =='DXB_train_val_test\\250-2970\\':
            data = np.loadtxt(r'datas\\MOT\\DXB\\250-2970\\pseudo_bbox.txt', delimiter=',',
                              dtype=int).tolist()  # todo
        # elif dataset =='SD\\8450-2180\\':
        ...
        for file in file_list[1:-1]:
            ...
            # print(label)
            # neg_samples_pic = [i for i in neg_samples if i[0] == img_id]
            # print(neg_sample)
            ...
            for h1 in range(0, img_h, w):
                for w1 in range(0, img_w, w):
                    ...
                    img_neg = np.ones((right-left, lower-upper, 1))  # 初始化矩阵
                    ...
                    #         img_neg[neg_sample[1]-left:neg_sample[3]-left, neg_sample[2]-upper:neg_sample[4]-upper, :] = 2
                    # print(img_neg[97:106, 0:5])
                    # pp
                    # print(cut_size)
                    img_save = np.zeros((cut_size, cut_size, 10))
                    img_save[:, :, 0:3] = img_pre_crop
                    img_save[:, :, 3:6] = img_crop
                    img_save[:, :, 6:9] = img_post_crop
                    img_save[:, :, 9:10] = img_neg

                    print(dataset)
                    np.save(os.path.join(save_path,
                                         'pseudo_label\\train\%03d_%s_%d_%d_%d.npy' % (save_id, dataset.replace(dataset.split('\\')[0]+'\\', '')[0:-1], img_id, left, upper)
                                         ),  # todo dataset[4:-1]
...
def convert_json(data, save_path, img_size=256):  # todo
    ...
    for file in os.listdir(os.path.join(save_path, 'pseudo_label\\train')):  # todo
        # if '001' in file:
        #     w, h = 400, 400
        # else:
        #     w, h = 400, 600
...
if __name__ == "__main__":
    ...
    save_path = [
        ...
        r'E:\Jiang\data\MOT\DXB_train_val_test',
    ]
    datasets = [
        ['SkySat\\002\\'],
        ['SD\\4570-3360\\', 'SD\\8450-2180\\'],
        ['DXB_train_val_test\\250-2970\\'],  #, 'DXB\\250-2970\\'
    ]
    cut_size = 256
    stride = 256
    padding = True
